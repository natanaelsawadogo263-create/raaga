...
import { fetchAdminCustomersList } from "@/lib/admin/data";
...
export default async function AdminClientsPage({ searchParams }: PageProps) {
  ...
  const { customers, error: fetchError } = await fetchAdminCustomersList(supabase);
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-xl font-black tracking-tight text-slate-900 sm:text-2xl">Clients</h1>
        </div>
        <Link
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
        <AdminCustomersListClient initialCustomers={customers} totalCount={customers.length} />
      </div>
    </div>
  );
}
